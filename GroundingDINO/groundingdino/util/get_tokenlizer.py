...
def get_pretrained_language_model(text_encoder_type, bert_base_uncased_path):
    if text_encoder_type == "bert-base-uncased":
        if os.path.exists(bert_base_uncased_path):
            print(f"Loading BERT model from local path: {bert_base_uncased_path}")
            return BertModel.from_pretrained(bert_base_uncased_path)
        else:
            print(f"Downloading BERT model from Hugging Face: {text_encoder_type}")
